...
exports.listAllProducts  = async (req, res) => {
    ...
    const produtos_reffor = req.params.produtos_reffor;
    ...
    const rows = await db.query('SELECT coalesce(venda.quantidade_peca,0) as qtd_pecas_vendidas, coalesce(venda.venda_total,0.00) as venda_total_venda, coalesce(venda.cod_fabricante,estoque.codigo::integer) as cod_fabricante, \r\
    ...
    case when (coalesce(coalesce(venda.procor_descricao_cor,estoque.procor_descricao_cor),'+trasso+')) = '+trasso+' then CASE WHEN(case when length(coalesce(venda.produto,estoque.item::integer)::character varying::text) = 6   \r\
    THEN substr(coalesce(venda.produto,estoque.item::integer)::character varying::text, 3, 1)   \r\
    ELSE substr(coalesce(venda.produto,estoque.item::integer)::character varying::text, 2, 1) END)::integer = 1 then '+catt+' else '+varr+' END  \r\
    else (coalesce(venda.procor_descricao_cor,estoque.procor_descricao_cor)) end as procor_descricao_cor, coalesce(venda.lojas_nome,estoque.lojas_nome)as lojas_nome  \r\
     from  \r\
     \r\
     ...
     (CASE WHEN preco_cheio < 0 THEN preco_cheio*-1 ELSE preco_cheio END) as preco_cheio1, produto, produtos_tamanho, tam_nom, produtos_cor,  \r\
     procor_descricao_cor, lojas_nome, nomefabrica \r\
     ...
     and cod_fabricante = '+codigofab+'  and produtos_reffor = '+aprodutos_reffor+'  and lojas_nome = '+alojanome+'  \r\
     group by cod_fabricante, for_nom, produtos_descricao, nome_revista, erp_produtos.produtos_pagina, num_produto,  \r\
     produtos_reffor, preco_cheio1,descricao_basica, produto, produtos_tamanho, tam_nom, produtos_cor, procor_descricao_cor \r\
            ,lojas_nome, nomefabrica order by num_produto)venda  \r\
            \r\
     full join \r\
     \r\
     ...
     item, produtos_tamanho, tam_nom, produtos_cor, procor_descricao_cor,lojas_nome, nomfant \r\
     FROM view_estoque  \r\
     ...
     left join erp_tamanho on (erp_tamanho.tam_cod = erp_produtos.produtos_tamanho) \r\
     left join erp_produtos_cor on (erp_produtos_cor.procor_cod = erp_produtos.produtos_cor) \r\
     where codigo::integer = '+codigofab+'  and produtos_reffor = '+aprodutos_reffor+'  and lojas_nome = '+alojanome+'  \r\
     group by codigo, nome_fornecedor, produtos_descricao, revista, erp_produtos.produtos_pagina, num_produto,  \r\
     produtos_reffor, item, produtos_tamanho, tam_nom, produtos_cor, procor_descricao_cor, desc_basica,lojas_nome, nomfant order by num_produto)estoque  \r\
     \r\
     on cod_fabricante = codigo::integer and venda.produto = estoque.item::integer group by venda.quantidade_peca,venda.venda_total, venda.cod_fabricante,estoque.codigo, \r\
     ...
     estoque.produtos_pagina, estoque.num_produto, estoque.produtos_reffor, estoque.item, estoque.produtos_tamanho, estoque.tam_nom, estoque.produtos_cor,  \r\
     estoque.procor_descricao_cor, venda.lojas_nome,estoque.lojas_nome order by produtos_tamanho')
